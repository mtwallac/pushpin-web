...
class Instagram(module.Module):
    ...
    def search(self, key, lat, lon, rad, since):
        ...
        url = 'https://api.instagram.com/v1/media/search' #?locationRadius={}km&order=date&publishedAfter={}&part=snippet&location={}&type=video&maxResults=50&key={}'
        while True:
            payload = { 'ACCESS_TOKEN': '8095144151.58559c5.4881fab5cf96478c83a7dba80d9bcfd9',
                       'LAT': lat,
                       'LNG': lon,
                       'DISTANCE': 1000}
            try:
                ...
                self.error("Unable to connect to Instagram API.")
                return []
            jsonobj = resp.json()
            if not jsonobj:
    ...
    def run(self, locname, lat, lon, rad, since):
        self.output('Collecting data from Instagram for {}...'.format(locname))
        startTime = datetime.now(timezone.utc).astimezone()

        pins = []
        count = 0

        apiKey = self.getKey('instagram_api')
